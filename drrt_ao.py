...
def calc_heur(neighbor,objective):
    return ed.transformed_distance(point_d_to_point_2(neighbor), objective) # For now the heuristic will be the euclidian distance from the objective
...
def oracle(V_near,Q_rand,G,team_objectives):
    ...
    for i in range(N):
        N=list(G[i].neighbors(V_near)) # A list of all neighbors of V_near in the roadmap
...
def find_path_in_tensor_roadmap(team):
    ...
    for i in range(N):
        path[i].insert(team_robots[i]) #inserting the starting position of each robot
    ...
    V_new = oracle(V_near,Q_rands,graphs_single_robots,team_objectives)
